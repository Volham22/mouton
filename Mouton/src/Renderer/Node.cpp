...
namespace Mouton {
                                    ...
                                    glm::mat4 parentTransform)
    {
        ...
        for (auto& child : m_Children)
            child->ProcessNodeHierarchy(animationName, animationProgress,
    ...
    void Node::AddMesh(const Mesh& mesh) { m_Meshes.push_back(mesh); }

    void Node::RetrieveBonesTransform(
        std::shared_ptr<std::array<glm::mat4, MTN_MAX_BONES_COUNT>>& transforms)
    {
        for (auto& bone : m_Bones)
