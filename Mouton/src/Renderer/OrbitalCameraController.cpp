...
namespace Mouton
{
    ...
    OrbitalCameraController::OrbitalCameraController(const std::shared_ptr<OrbitalCamera>& cam, float cameraSpeed)
        : m_Camera(cam), m_HorizontalAngle(1.0f), m_VerticalAngle(1.0f), m_Distance(20.0f), m_CameraSpeed(cameraSpeed),
          m_LastMousePosition(1.0f)
    {
    }

    OrbitalCameraController::OrbitalCameraController(float cameraSpeed)
        : m_Camera(), m_HorizontalAngle(1.0f), m_VerticalAngle(1.0f), m_Distance(1.0f), m_CameraSpeed(cameraSpeed),
          m_LastMousePosition(1.0f)
    {
    }
    ...
    {
        EventSystem::ApplyFunction<MouseMovedEvent>(&event, [this](Event& event) -> bool {
        ...
        EventSystem::ApplyFunction<MouseScrolledEvent>(&event, [this](Event& event) -> bool {
            MouseScrolledEvent& ev = reinterpret_cast<MouseScrolledEvent&>(event);
