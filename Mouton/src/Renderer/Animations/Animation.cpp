...
namespace Mouton {
    ...
    Animation::Animation()
        : m_Name(), m_Duration(0.0), m_TickPerSecond(25.0), m_Tick(0.0),
          m_Begin(), m_Playing(false)
    {}

    Animation::Animation(const std::string& name, double duration,
                         double tickPerSecond)
        : m_Name(name), m_Duration(duration),
          m_TickPerSecond(tickPerSecond != 0.0 ? tickPerSecond : 25.0),
    ...
    double Animation::Update()
    {
        ...
        auto elapsed = duration<double>(duration_cast<milliseconds>(
