...
namespace Mouton {
    ...
    ModelLoader::CreateModelLoader(const std::string& path)
    {
    ...
    Model::Model(const std::shared_ptr<std::vector<Animation>>& animations,
    ...
    void Model::PlayAnimation(const std::string& name)
    {
        if (!m_CurrentAnim)
        {
            ...
            MTN_WARN("Animation {0} is already playing !",
                     m_CurrentAnim->GetName().c_str());
    }
    ...
    void Model::DrawModel()
    {
        if (m_CurrentAnim)
        {
            ...
            if (animProgress >= 0)
            {
                ...
                m_RootNode->ProcessNodeHierarchy(
                    m_CurrentAnim->GetName(), animProgress,
                    m_RootNode->GetTransformation());
            } else
                StopAnimation();
        }
    ...
    Model::GetBonesTransforms() const
    {
        auto transforms
            = std::make_shared<std::array<glm::mat4, MTN_MAX_BONES_COUNT>>();
