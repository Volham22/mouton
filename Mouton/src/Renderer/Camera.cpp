...
namespace Mouton {
    ...
    {
        m_Rotation = glm::rotate(glm::mat4(1.0f), glm::radians(angle),
                                 glm::vec3(0, 0, 1));
        m_View = glm::inverse(m_Position * m_Rotation);
        m_VP = m_Proj * m_View;
    }

    void OrthographicCamera::SetCoords(float left, float right, float top,
    ...
    PerspectiveCamera::PerspectiveCamera(float aspectRatio, float fov,
        ...
        : m_Position(position), m_Direction(direction), m_Projection(1.0f),
          m_VP(1.0f)
    {
        m_Projection
            = glm::perspective(glm::radians(fov), aspectRatio, 0.1f, 100.0f);
        m_VP = m_Projection
               * glm::lookAt(position, glm::normalize(m_Position - m_Direction),
                             glm::vec3(0, 0, 1));
    }
    ...
    void PerspectiveCamera::SetPosition(const glm::vec3& position)
    {
        m_Position = position;
        m_VP = m_Projection
               * glm::lookAt(m_Position, m_Direction, glm::vec3(0, 0, 1));
    }
    ...
    OrbitalCamera::OrbitalCamera(float aspectRatio, float fov,
                                 const glm::vec3& targetPos, float near,
                                 float far)
        : m_Projection(
            glm::perspective(glm::radians(fov), aspectRatio, near, far)),
          m_VP(m_Projection
               * glm::lookAt(targetPos + 1.0f, targetPos,
    ...
    void OrbitalCamera::Orbit(float angleHorizontal, float angleVertical,
                              float distance)
    {
        glm::vec3 horizontal
            = { glm::cos(glm::radians(angleHorizontal)),
                glm::sin(glm::radians(angleHorizontal)), 0.0f };
        glm::vec3 vertical = { 0.0f, glm::cos(glm::radians(angleVertical)),
                               glm::sin(glm::radians(angleVertical)) };

        glm::mat4 view
            = glm::lookAt(glm::normalize(horizontal + vertical) * distance,
