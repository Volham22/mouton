...
static void ImGui_ImplGlfw_UpdateMousePosAndButtons()
{
    ...
    for (int n = 0; n < platform_io.Viewports.Size; n++)
    {
        ...
        if (focused)
        {
            ...
            {
                glfwSetCursorPos(
                    window, (double)(mouse_pos_backup.x - viewport->Pos.x),
            ...
            } else
            {
                ...
                if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
                {
                    ...
                    io.MousePos = ImVec2((float)mouse_x + window_x,
        ...
        // not set by the back-end, imgui will ignore this field and infer the
...
static void ImGui_ImplGlfw_WindowSizeCallback(GLFWwindow* window, int, int)
{
    if (ImGuiViewport* viewport = ImGui::FindViewportByPlatformHandle(window))
    {
        ...
        {
            // GLFW may dispatch window size event after calling
            // glfwSetWindowSize(). However depending on the platform the
            // callback may be invoked at different time: on Windows it appears
            // to be called within the glfwSetWindowSize() call whereas on Linux
            // it is queued and invoked during glfwPollEvents(). Because the
            // event doesn't always fire on glfwSetWindowSize() we use a frame
            // counter tag to only ignore recent glfwSetWindowSize() calls.
            bool ignore_event = (ImGui::GetFrameCount()
...
static void ImGui_ImplGlfw_CreateWindow(ImGuiViewport* viewport)
{
    ...
    data->Window
        = glfwCreateWindow((int)viewport->Size.x, (int)viewport->Size.y,
    ...
    glfwSetWindowCloseCallback(data->Window,
                                        ...
                                        LPARAM lParam)
{
    if (msg == WM_NCHITTEST)
    {
        // Let mouse pass-through the window. This will allow the back-end to
        // set io.MouseHoveredViewport properly (which is OPTIONAL). The
