...
namespace Mouton {
    ...
    AssimpModelLoader::AssimpModelLoader(const std::string& path)
        : m_LoadedModel(), m_Animations(new std::vector<Animation>()),
          m_Path(path), m_BonesIndex(0), m_Bones(), m_BoneNode(), m_Textures()
    {}
    ...
    bool AssimpModelLoader::Load()
    {
        ...
        if (!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE
            || !scene->mRootNode)
        {
            MTN_ERROR("Scene {0} couldn't be loaded ! Reason: {1}",
    ...
    std::shared_ptr<std::vector<Animation>>&
    AssimpModelLoader::GetModelAnimations()
    {
        return m_Animations;
    }
                                      ...
                                      std::shared_ptr<Node>& currentNode)
    {
        ...
        {
            auto child
                = std::make_shared<Node>(ToGlmMat4(node->mTransformation));
                                     ...
                                     std::shared_ptr<Node>& node)
    {
        ...
        for (unsigned int i = 0; i < mesh->mNumVertices; i++)
        {
            ...
            // Make the ID to -1 by default, it is required for weight
            // assignment by LoadMeshBones
        ...
        if (mesh->mMaterialIndex > 0)
        {
            ...
            auto texDiff = LoadTextures(mat, aiTextureType_DIFFUSE,
                                        TextureType::Diffuse);
            auto texSpec = LoadTextures(mat, aiTextureType_SPECULAR,
                                        TextureType::Specular);
            auto texNorm
                = LoadTextures(mat, aiTextureType_HEIGHT, TextureType::Normal);
            auto texHei
                = LoadTextures(mat, aiTextureType_AMBIENT, TextureType::Height);
        ...
        LoadMeshBones(mesh, meshNode, scene, vertices,
                      node); // Load mesh bones if any

        return Mesh(vertices, indices, textures,
                    mesh->mNumBones > 0 ? true : false);
    }
                                          ...
                                          std::shared_ptr<Node>& node)
    {
        for (unsigned int i = 0; i < mesh->mNumBones; i++)
        {
            ...
            if (it == m_Bones.end())
            {
                auto meshBone = std::make_shared<Bone>(
                    ++m_BonesIndex, boneName, ToGlmMat4(bone->mOffsetMatrix));
                meshBone->SetGlobalTransformMatrix(
                    ToGlmMat4(scene->mRootNode->mTransformation));
            ...
            for (unsigned int j = 0; j < bone->mNumWeights; j++)
            {
                ...
                // We support only 4 bones to affect a vertex, if it's more than
                // 4 then it will be undefined behaviour. A warning may be
                // emmitted
                ...
                if (bonePos < 4)
                {
                    vertices[vertexID].bonesID[bonePos]
                        = m_Bones[boneName]->GetIndex();
                    vertices[vertexID].weight[bonePos]
                        = bone->mWeights[j].mWeight;
                } else
                    MTN_WARN(
                        "Bone {0} affect more than 4 vertices !",
                        boneName.c_str()); // TODO: Take the most relevant one
            }
        }
    }
        ...
        if (scene->mNumAnimations > 0)
        {
            for (unsigned int i = 0; i < scene->mNumAnimations; i++)
            {
                ...
                m_Animations->push_back(Animation(
                    animName.length() > 0 ? animName : "animation" + i,
                ...
                for (unsigned int j = 0; j < animation->mNumChannels; j++)
                {
                    ...
                    for (unsigned int k = 0;
                         k < animation->mChannels[j]->mNumPositionKeys; k++)
                    {
                        aiNode* animNode = scene->mRootNode->FindNode(
                            animation->mChannels[j]->mNodeName);

                        glm::vec3 position;
                        position.x = animation->mChannels[j]
                                         ->mPositionKeys[k]
                                         .mValue.x;
                        position.y = animation->mChannels[j]
                                         ->mPositionKeys[k]
                                         .mValue.y;
                        position.z = animation->mChannels[j]
                                         ->mPositionKeys[k]
                                         .mValue.z;
                        ...
                        glm::quat rotation;
                        rotation.x = animation->mChannels[j]
                                         ->mRotationKeys[k]
                                         .mValue.x;
                        rotation.y = animation->mChannels[j]
                                         ->mRotationKeys[k]
                                         .mValue.y;
                        rotation.z = animation->mChannels[j]
                                         ->mRotationKeys[k]
                                         .mValue.z;
                        rotation.w = animation->mChannels[j]
                                         ->mRotationKeys[k]
                                         .mValue.w;

                        double time
                            = animation->mChannels[j]->mPositionKeys[k].mTime;

                        key = KeyFrame(animName, position, scale, rotation,
                                       time);

                        auto it = m_Bones.find(animNode->mName.C_Str());
                        if (it != m_Bones.end())
                            m_Bones[animNode->mName.C_Str()]->AddKeyFrame(
                                animName, key);
                    }
                }
            }
    ...
    AssimpModelLoader::LoadTextures(aiMaterial* mat, aiTextureType aiType,
                                    TextureType type)
    {
        std::vector<MeshTexture> textures;
        ...
        for (unsigned int i = 0; i < mat->GetTextureCount(aiType); i++)
        {
            ...
            {
                auto tex
                    = Texture2D::CreateTexture((m_Path + path.C_Str()).c_str());

                m_Textures[path.C_Str()] = std::make_pair(type, tex);
