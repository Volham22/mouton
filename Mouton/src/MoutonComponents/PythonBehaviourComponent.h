...
namespace Mouton {
    ...
    enum class PythonBehaviourType : uint16_t {
        Unknown = 0,
        SpriteBehaviour = 1,
        OrthographicCameraBehaviour = 2
    };

    // Python Behaviour Component is here to hold any PythonBinder
    // implementation in order to be a component in the ECS.
    template<typename T>
    class PythonBehaviourComponent : public Component
    {
      public:
        static_assert(std::is_base_of_v<PythonBinder, T>,
                      "T must implements PythonBinder interface !");

        std::unique_ptr<PythonBinder> pythonBehaviour;

        template<typename... Args>
        PythonBehaviourComponent(const std::string& name, Args... args)
            : Component(Component::ComponentType::PythonBehaviourComponent,
                        name),
              pythonBehaviour(std::make_unique<T>(args...))
        {}

        static inline PythonBehaviourComponent<PythonBinder>&
        ToPythonBehaviourComponent(Component& comp)
        {
        ...
        void Serialize(Writer& writer)
        {
            ...
            if (pythonBehaviour->GetBoundComponent())
                writer.String(pythonBehaviour->GetBoundComponent()
                                  ->GetComponentName()
                                  .c_str());
            else
                writer.Null();
        ...
        {
            constexpr size_t SPRITE_BEHAVIOUR_HASH
                = Utils::StringHash::HashString("SpriteComponent");
            constexpr size_t ORTHO_BEHAVIOUR_HASH
                = Utils::StringHash::HashString("OrthographicCameraComponent");

            const size_t valueTypeHash = Utils::StringHash::HashString(
                std::string(value["Type"].GetString()));

            switch (valueTypeHash)
            {
            case SPRITE_BEHAVIOUR_HASH: {
                auto* ptr
                    = new PythonBehaviourComponent<SpriteComponentScriptable>(
                        value["Name"].GetString(),
                        std::string(value["ModuleName"].GetString()), nullptr,
                        nullptr);
            ...
            case ORTHO_BEHAVIOUR_HASH: {
                auto* ptr = new PythonBehaviourComponent<
            ...
            default:
                MTN_ERROR("Unknown behaviour type {} while loading scene !",
