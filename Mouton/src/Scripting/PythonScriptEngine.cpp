...
PYBIND11_EMBEDDED_MODULE(Mouton, m)
{
    ...
    .def("__getitem__", [](const glm::vec2& vec, uint32_t key) { return vec[key]; })
    ...
    .def("__getitem__", [](const glm::vec3& vec, uint32_t key) { return vec[key]; })
    ...
    .def("__getitem__", [](const glm::vec4& vec, uint32_t key) { return &vec[key]; })
